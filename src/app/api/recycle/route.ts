    ...
    if (geminiKey) {
        try {
            ...
            const prompt = `
                ...
                [지시사항]
                1. 당신의 주요 임무는 **대형 폐기물(가구, 가전 등)의 올바른 처리 방법을 안내**하는 것입니다.
                2. 질문이 대형 폐기물 관련이면 [2. 대형폐기물] 데이터를 활용하여 **규격별 수수료 가격**을 먼저 알려주세요.
                3. **신청 방법**을 반드시 포함하세요 (예: 관할 구청 홈페이지 신청, 지정 판매소 스티커 구매 등).
                4. [1. 공공데이터]를 참고하여 재활용 가능 여부를 확인하고, 불가능하다면 대형 폐기물로 처리해야 함을 강조하세요.
                5. **핵심 정보 위주로 간결하게 요약**하여 답변하세요. 불필요한 서술은 생략하고 결론부터 말씀하세요.
                6. 정보가 부족하더라도 일반 상식을 동원해 **대형 폐기물 처리 요령**을 짧게 제시하세요.
